...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        ProductLogic::insert_schedule($food->store_id, $food->id);
...
//            }
    ...
    public function add_schedule(Request $request)
    {
        ...
        $temp = ItemSchedule::where('day', $request->day)->where('store_id', Helpers::get_store_id())
        ...
        $store = Helpers::get_store_data();
        $item_schedule = ItemSchedule::insert(['store_id'=>Helpers::get_store_id(),'item_id'=>$request->product_id,'day'=>$request->day,'opening_time'=>$request->start_time,'closing_time'=>$request->end_time]);
        $product = Item::where('id', $request->product_id)->first();
        
        return response()->json([
            'view' => view('vendor-views.product.partials._schedule', compact('product'))->render(),
        ]);
    }
    ...
    {
        $store = Helpers::get_store_data();
        $schedule = ItemSchedule::where('store_id', $store->id)->find($item_schedule);
        $product = Item::where('id', $schedule->item_id)->first();
        if(!$schedule)
        {
