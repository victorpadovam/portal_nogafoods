...
class VendorLoginController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        // Dados principais
        $store->establishment_name = $request->establishment_name;
        $store->cnpj = $request->cnpj;
        $store->social_reason = $request->social_reason ?? null;
        $store->city = $request->city ?? null;
        $store->address = $request->address ?? null;
        $store->neighborhood = $request->neighborhood ?? null;
        $store->number = $request->number ?? null;
        $store->cep = $request->cep ?? null;
        $store->state = $request->state ?? null;
        $store->module_id = $request->module_id ?? null;

        // Step 3
        $store->firstPhone = $request->first_phone ?? null;
        $store->secondPhone = $request->second_phone ?? null;
        $store->whatsapp = $request->whatsapp ?? null;
        $store->socialNetwork = $request->social_network ?? null;


        // Step 5
        $store->businessType = $request->business_type ?? null;
        $store->establishmentCategory = $request->establishment_category ?? null;
        $store->serviceType = $request->service_type ?? null;
        $store->hasDeliveryService = filter_var($request->has_delivery_service, FILTER_VALIDATE_BOOLEAN);
        $store->hasMercadoPago = filter_var($request->has_mercado_pago, FILTER_VALIDATE_BOOLEAN);
        $store->tipo_de_pagamento_da_loja = json_decode($request->tipo_de_pagamento_da_loja, true);
        $store->tempo_aproximada_para_delivery = $request->tempo_aproximada_para_delivery ?? null;
        $store->tempo_aproximada_para_retirada = $request->tempo_aproximada_para_retirada ?? null;
        $store->pagamento_padrao = filter_var($request->pagamento_padrao, FILTER_VALIDATE_BOOLEAN);
        $store->minimumOrder = $request->minimum_order ?? null;
        $store->deliveryFee = $request->delivery_fee ?? null;
        $store->listaHorarioDeFuncionamentoDaLoja = json_decode($request->listaHorarioDeFuncionamentoDaLoja, true);
        ...
        if ($request->hasFile('cover_photo')) {
            $store->cover_photo = Helpers::upload('store/', 'png', $request->file('cover_photo'));
        }
        if ($request->hasFile('destaque')) {
            $store->destaque = Helpers::upload('store/', 'png', $request->file('destaque'));
        }
        if ($request->hasFile('catalogo')) {
            $store->catalogo = Helpers::upload('store/', 'png', $request->file('catalogo'));
        }
        if ($request->hasFile('owenerDocumentoComFotoFrente')) {
            $store->owenerDocumentoComFotoFrente = Helpers::upload('store/documents/', 'png', $request->file('owenerDocumentoComFotoFrente'));
        }
        if ($request->hasFile('owenerDocumentoComFotoVerso')) {
            $store->owenerDocumentoComFotoVerso = Helpers::upload('store/documents/', 'png', $request->file('owenerDocumentoComFotoVerso'));
        }
       ...
       // Dados do Step 7
        ...
        // Step 8 (dono)
        // Dados do Step 8
        $store->ownerName = $request->owner_name ?? null;
        $store->ownerCargo = $request->owner_cargo ?? null;
        $store->ownerEmail = $request->owner_email ?? null;
        $store->ownerPhone = $request->owner_phone ?? null;
        $store->observation = $request->observation ?? null;
