...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        ProductLogic::insert_schedule($item->store_id, $item->id);
        ...
        if (Helpers::get_mail_status('product_approval') && data_get($product_approval_datas, 'Add_new_product', null) == 1) {
            ...
            return response()->json(['message' => translate('messages.The_product_will_be_published_once_it_receives_approval_from_the_admin.')], 200);
                   ...
                   return response()->json([
                'message' => translate('messages.product_added_successfully'),
                'item_id' => $item->id
            ], 200);
    ...
    {
        try {
            $item = Item::withoutGlobalScope('translate')->with('tags')->where('id', $id)->first();
            $item_schedule = ItemSchedule::where('item_id', $item->id)->get();
            $item->schedules = $item_schedule;
    ...
    public function delete(Request $request)
    {
        ...
        ProductLogic::delete_schedule($product->store_id, $product->id);
    ...
    public function get_schedule(Request $request, $product_id)
    {
        if (!$request->vendor->stores[0]->item_section) {
            return response()->json([
                'errors' => [
        ...
        $item_schedule = ItemSchedule::where('store_id', $store_id)->where('item_id', $product_id)->get();

        return response()->json($item_schedule, 200);
    }
...
$temp = ItemSchedule::where('day', $request->day)
    ->where('store_id', $store_id)
    ->where('item_id', $request->item_id)
    ->where(function ($q) use ($request) {
        return $q->where(function ($query) use ($request) {
            return $query->where('opening_time', '<=', $request->start_time)
                         ->where('closing_time', '>=', $request->start_time);
        })->orWhere(function ($query) use ($request) {
            return $query->where('opening_time', '<=', $request->end_time)
                         ->where('closing_time', '>=', $request->end_time);
        });
    })
    ->first();

if (isset($temp)) {
    // Atualiza se o mesmo dia/item e intervalo já existirem
    $temp->opening_time = $request->start_time;
    $temp->closing_time = $request->end_time;
    $temp->save();

    return response()->json([
        'message' => translate('messages.Schedule updated successfully'),
        'id' => $temp->id,
    ], 200);
}


        $item_schedule = ItemSchedule::insertGetId(['store_id' => $store_id, 'item_id' => $request->item_id, 'day' => $request->day, 'opening_time' => $request->start_time, 'closing_time' => $request->end_time]);

        return response()->json(['message' => translate('messages.Schedule added successfully'), 'id' => $item_schedule], 200);
    }

    public function remove_schedule(Request $request, $item_schedule)
    {
        if (!$request->vendor->stores[0]->item_section) {
            return response()->json([
                'errors' => [
        ...
        $schedule = ItemSchedule::where('store_id', $store_id)->find($item_schedule);
        if (!$schedule) {
        ...
        return response()->json(['message' => translate('messages.product_schedule_deleted_successfully')], 200);
    }
}
