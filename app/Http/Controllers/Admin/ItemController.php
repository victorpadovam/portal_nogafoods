...
// dd($request?->sub_tab,$foods,);
    ...
    {
        $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('store', 'category', 'module')->findOrFail($request->product_id);
        ...
        $temp = ItemSchedule::where('day', $request->day)->where('store_id', $store_id)
        ...
        $item_schedule = ItemSchedule::insert(['store_id' => $store_id, 'item_id' => $request->product_id, 'day' => $request->day, 'opening_time' => $request->start_time, 'closing_time' => $request->end_time]);
        $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('store', 'category', 'module')->findOrFail($request->product_id);

        return response()->json([
            'view' => view('admin-views.product.partials._schedule', compact('product'))->render(),
        ]);
    }

    public function remove_schedule($item_schedule)
    {
        $schedule = ItemSchedule::where('id', $item_schedule)->first();
        ...
        $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('store', 'category', 'module')->findOrFail($schedule->item_id);
