...
class BannerLogic
{
    public static function get_banners($zone_id, $featured = false)
    {
        ...
        $banners = $banners->where(function ($query) use ($zone_id) {
                $query->whereIn('zone_id', json_decode($zone_id, true))->orWhere('zone_id', 0);
