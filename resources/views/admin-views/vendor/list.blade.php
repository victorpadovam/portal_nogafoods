...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card card--bg-1">
                    ...
                    <h4 class="title">{{ $total_store ?? 0 }}</h4>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card card--bg-2">
                    ...
                    <h4 class="title">{{ $active_stores ?? 0 }}</h4>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card card--bg-3">
                    ...
                    <h4 class="title">{{ $inactive_stores ?? 0 }}</h4>
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="resturant-card card--bg-4">
                    @php
                        $data = \App\Models\Store::where('created_at', '>=', now()->subDays(30))
                            ->where('module_id', Config::get('module.current_module_id'))
                            ->count();
                    @endphp

                    <h4 class="title">{{ $data ?? 0 }}</h4>
        ...
        <!-- Approval Flow Card -->
        <div class="card mb-3 approval-flow-card">
            <div class="card-body py-3 px-4 d-flex align-items-center justify-content-between flex-wrap">
                ...
                <div class="d-flex align-items-center flex-wrap">
                    ...
                    <!-- Status -->
                    <div class="approval-status-box mr-3 mb-2">
                        <div class="d-flex align-items-center">
                            <div class="approval-icon mr-2">
                                <i class="fas fa-shield-alt"></i>
                            </div>
                            <div>
                                <div class="approval-label">STATUS DE CADASTRO</div>
                                <div class="approval-title">Fluxo de Aprovação</div>
                            </div>
                        </div>
                    </div>
                    <button type="button" id="btnAutomatic" class="btn approval-btn mr-2 mb-2">
                        <i class="tio-flash mr-1"></i>
                        APROVAÇÃO AUTOMÁTICA
                    </button>

                    <button type="button" id="btnManual" class="btn approval-btn mb-2">
                ...
                <!-- Right Side -->
                <div class="approval-saved d-flex align-items-center mt-2 mt-md-0">
                    <span class="approval-check mr-2">
                        <i class="tio-checkmark-circle"></i>
                    </span>
                    <span class="approval-saved-text">
                        Preferência de gestão salva.
                    </span>
                </div>
                                ...
                                <td>
                                    <div class="btn--container justify-content-center">
                                        <div class="btn--container justify-content-center">
                                            ...
                                            @if (isset($fluxo) && $fluxo->tipo === 'manual' && $store->created_at >= $fluxo->updated_at)
                                                @if ($store->status_de_aprovacao === 'aprovado')
                                                    <!-- ✅ Ícone de aprovado -->
                                                    <span class="btn action-btn btn--success btn-outline-success"
                                                        data-bs-toggle="tooltip" data-bs-placement="top"
                                                        title="Loja já aprovada">
                                                        <i class="tio-checkmark-circle"></i>
                                                    </span>
                                                @else
                                                    <!-- 🔔 Botão aprovar -->
                                                    <a href="javascript:void(0);"
                                                        class="btn action-btn btn--warning btn-outline-warning aprovar-loja"
                                                        data-id="{{ $store->id }}" data-bs-toggle="tooltip"
                                                        data-bs-placement="top" title="Aprovar cadastro da loja">
...
@push('script_2')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.aprovar-loja').forEach(button => {
                button.addEventListener('click', function() {

                    let storeId = this.dataset.id;

                    fetch('/api/v1/aprovar', {
                            method: 'POST',
                            headers: {
                        ...
                        .then(data => {
                            ...
                            // 🔥 Espera 4 segundos antes de recarregar
                            setTimeout(() => {
                                location.reload();
                            }, 3000);
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function ativarBotao(tipo) {
                if (tipo === 'automatico') {
                    btnAutomatic.classList.add("btn-active");
                    btnManual.classList.remove("btn-active");
                } else if (tipo === 'manual') {
                    btnManual.classList.add("btn-active");
                    btnAutomatic.classList.remove("btn-active");
                }
            }

            function carregarFluxo() {
                fetch('/api/v1/aprovacao-buscar')
                    .then(response => response.json())
                    .then(data => {
                        if (data.tipo) {
                            ativarBotao(data.tipo);
                        }
                    })
                    .catch(error => console.error('Erro ao carregar:', error));
            }

            function salvarFluxo(tipo) {

                fetch('/api/v1/aprovacao', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': token
            ...
            // 🔥 IMPORTANTE: carregar ao abrir a página
            carregarFluxo();
